...
/**
 * The Apple logo, as Apple draws it.
 *
 ...
 * colour, stays crisp at any size, and cannot fail to load on the one screen
 * where somebody is deciding whether to trust us with a card.
 */
export function AppleMark({ className, size = 14 }: { className?: string; size?: number }) {
  return (
    <svg
      viewBox="0 0 24 24"
      width={size}
...
/** "&#63743; Pay", set as one unbreakable unit so it never wraps mid-mark. */
export function ApplePayMark({ className }: { className?: string }) {
  return (
    <span className={cx("inline-flex items-center gap-0.5 whitespace-nowrap", className)}>
      <AppleMark size={13} />
      <span>Pay</span>
    </span>
  );
